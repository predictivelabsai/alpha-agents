...
class YFinanceUtil:
    ...
    def get_sector_universe(self, sector: str, market: str = 'US', max_market_cap: float = None) -> List[str]:
    ...
    def _get_us_sector_stocks(self, sector: str, max_market_cap: float = None) -> List[str]:
        ...
        sector_stocks = {
            'Technology': [
                ...
                'LRCX', 'KLAC', 'MCHP', 'FTNT', 'PANW', 'CRWD', 'ZS', 'OKTA', 'DDOG', 'NET'
            ],
            'Healthcare': [
                'JNJ', 'UNH', 'PFE', 'ABT', 'TMO', 'MDT', 'DHR', 'BMY', 'ABBV', 'MRK',
                'LLY', 'GILD', 'AMGN', 'CVS', 'CI', 'ANTM', 'HUM', 'CNC', 'MOH', 'ELV',
                'ISRG', 'SYK', 'BSX', 'ZBH', 'BAX', 'BDX', 'EW', 'HOLX', 'DXCM', 'VEEV'
            ],
            'Financial Services': [
                'JPM', 'BAC', 'WFC', 'GS', 'MS', 'C', 'AXP', 'BLK', 'SCHW', 'USB',
                'PNC', 'TFC', 'COF', 'CME', 'ICE', 'SPGI', 'MCO', 'AON', 'MMC', 'AJG',
                'CB', 'TRV', 'PGR', 'ALL', 'MET', 'PRU', 'AIG', 'AFL', 'HIG', 'FIS'
            ],
            'Consumer Cyclical': [
                'AMZN', 'TSLA', 'HD', 'NKE', 'MCD', 'SBUX', 'LOW', 'TJX', 'F', 'GM',
                'BKNG', 'MAR', 'RCL', 'CCL', 'NCLH', 'LVS', 'MGM', 'WYNN', 'CZR', 'PENN',
                'EBAY', 'ETSY', 'W', 'CHWY', 'RVLV', 'LULU', 'ULTA', 'TPG', 'KMX', 'AN'
            ],
            'Communication Services': [
                'GOOGL', 'META', 'DIS', 'NFLX', 'CMCSA', 'VZ', 'T', 'TMUS', 'CHTR', 'DISH',
                'TWTR', 'SNAP', 'PINS', 'ROKU', 'SPOT', 'ZM', 'DOCU', 'TEAM', 'WORK', 'PTON',
                'MTCH', 'BMBL', 'RBLX', 'U', 'FUBO', 'SIRI', 'LBRDK', 'LBRDA', 'FWONK', 'BATRK'
            ],
            'Industrial': [
                'BA', 'CAT', 'GE', 'MMM', 'HON', 'UPS', 'RTX', 'LMT', 'NOC', 'GD',
                'FDX', 'UNP', 'CSX', 'NSC', 'DAL', 'UAL', 'AAL', 'LUV', 'JBLU', 'ALK',
                'DE', 'EMR', 'ETN', 'PH', 'ITW', 'ROK', 'DOV', 'XYL', 'FTV', 'GNRC'
            ],
            'Consumer Defensive': [
                'WMT', 'PG', 'KO', 'PEP', 'COST', 'WBA', 'CVS', 'TGT', 'KR', 'CL',
                'GIS', 'K', 'CPB', 'SJM', 'HSY', 'MDLZ', 'MNST', 'KDP', 'STZ', 'BF-B',
                'PM', 'MO', 'BTI', 'EL', 'CLX', 'CHD', 'CAG', 'HRL', 'MKC', 'LW'
            ],
            'Energy': [
                'XOM', 'CVX', 'COP', 'EOG', 'SLB', 'PSX', 'VLO', 'MPC', 'OXY', 'BKR',
                'HAL', 'DVN', 'FANG', 'APA', 'EQT', 'CTRA', 'MRO', 'HES', 'KMI', 'OKE',
                'WMB', 'EPD', 'ET', 'MPLX', 'PAA', 'TRGP', 'AM', 'SUN', 'DINO', 'SM'
            ],
            'Utilities': [
                'NEE', 'DUK', 'SO', 'D', 'AEP', 'EXC', 'XEL', 'SRE', 'PEG', 'ED',
                'ETR', 'WEC', 'ES', 'FE', 'AWK', 'ATO', 'CMS', 'DTE', 'NI', 'LNT',
                'EVRG', 'CNP', 'AES', 'PPL', 'PNW', 'IDA', 'UGI', 'NJR', 'SWX', 'OGE'
            ],
            'Real Estate': [
                'AMT', 'PLD', 'CCI', 'EQIX', 'PSA', 'WELL', 'DLR', 'O', 'SBAC', 'EXR',
                'AVB', 'EQR', 'VTR', 'ESS', 'MAA', 'UDR', 'CPT', 'FRT', 'BXP', 'VNO',
                'KIM', 'REG', 'SPG', 'MAC', 'SLG', 'HST', 'RHP', 'PK', 'APLE', 'AIV'
            ],
            'Materials': [
                'LIN', 'APD', 'SHW', 'FCX', 'NEM', 'DOW', 'DD', 'PPG', 'ECL', 'IFF',
                'ALB', 'CE', 'VMC', 'MLM', 'PKG', 'IP', 'WRK', 'SON', 'SEE', 'AVY',
                'CF', 'FMC', 'LYB', 'EMN', 'RPM', 'SUM', 'KRA', 'SLVM', 'HUN', 'OLN'
            ]
        }
    ...
    def _get_china_sector_stocks(self, sector: str, max_market_cap: float = None) -> List[str]:
        """Get China stocks for a specific sector"""
        # Sample China stocks by sector (would need expansion)
        china_sector_stocks = {
            'Technology': [
                'BABA', '9988.HK', 'JD', '9618.HK', 'BIDU', '9888.HK', 'TCEHY', '0700.HK',
                'PDD', 'NTES', '9999.HK', 'TME', 'BILI', 'IQ', 'DOYU', 'HUYA'
            ],
            'Healthcare': [
                'WuXi AppTec', '2359.HK', 'BeiGene', '6160.HK', 'Innovent Bio', '1801.HK'
            ],
            'Financial Services': [
                'ICBC', '1398.HK', 'CCB', '0939.HK', 'ABC', '1288.HK', 'BOC', '3988.HK'
            ],
            'Consumer Cyclical': [
                'NIO', 'XPEV', 'LI', 'BEKE', 'EDU', 'TAL', 'YMM', 'VIPS'
            ],
            'Energy': [
                'PetroChina', '0857.HK', 'Sinopec', '0386.HK', 'CNOOC', '0883.HK'
            ]
        }
    ...
    def get_fundamental_metrics(self, ticker: str) -> Dict:
        ...
        try:
            ...
            metrics = {
                ...
                # Growth metrics
                'revenue_growth_5y': self._calculate_growth_rate(financials, 'Total Revenue', 5),
                'net_income_growth_5y': self._calculate_growth_rate(financials, 'Net Income', 5),
                'cash_flow_growth_5y': self._calculate_growth_rate(cash_flow, 'Operating Cash Flow', 5),
                ...
                'debt_service_ratio': self._calculate_debt_service_ratio(financials, cash_flow),
                ...
                'growth_consistency': self._check_growth_consistency(financials, cash_flow),
                'profitability_consistency': self._check_profitability_consistency(financials),
    ...
    def _calculate_growth_rate(self, df: pd.DataFrame, metric: str, years: int) -> float:
    ...
    def _calculate_roic(self, financials: pd.DataFrame, balance_sheet: pd.DataFrame) -> float:
    ...
    def _calculate_debt_to_ebitda(self, financials: pd.DataFrame, balance_sheet: pd.DataFrame) -> float:
    ...
    def _calculate_debt_service_ratio(self, financials: pd.DataFrame, cash_flow: pd.DataFrame) -> float:
    ...
    def _check_growth_consistency(self, financials: pd.DataFrame, cash_flow: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'overall_consistent': all([revenue_growth > 0, income_growth > 0, cf_growth > 0])
            }
        except Exception:
    ...
    def _check_profitability_consistency(self, financials: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            net_incomes = financials.loc['Net Income'].dropna() if 'Net Income' in financials.index else pd.Series()
            ...
            return {
                ...
                'overall_consistent': margin_trend >= -1 and all(m > 0 for m in margins[-3:])  # Recent margins positive
            }
        except Exception:
            return {'overall_consistent': False}
    
    def _assess_debt_health(self, balance_sheet: pd.DataFrame, financials: pd.DataFrame) -> Dict:
    ...
    def _calculate_fundamental_score(self, metrics: Dict) -> float:
        ...
        try:
            ...
            total_score = growth_score + profitability_score + debt_score + valuation_score
            return round(total_score, 1)
            
        except Exception:
            return 0
    
    def screen_stocks(self, sector: str, market: str = 'US', criteria: Dict = None) -> List[Dict]:
        """Screen stocks based on fundamental criteria"""
        try:
            # Get stock universe
            tickers = self.get_sector_universe(sector, market, criteria.get('max_market_cap'))
    ...
    def _meets_criteria(self, metrics: Dict, criteria: Dict) -> bool:
        ...
        try:
            ...
            if criteria.get('min_revenue_growth', 0) > metrics.get('revenue_growth_5y', 0):
                return False
            ...
            if criteria.get('max_debt_to_ebitda', float('inf')) < metrics.get('debt_to_ebitda', 0):
                return False
            
            # Market cap criteria
            if criteria.get('max_market_cap') and metrics.get('market_cap', 0) > criteria.get('max_market_cap'):
                return False
